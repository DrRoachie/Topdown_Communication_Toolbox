...
% strong influence of SNR on PSI so we collapse across all levels of SNR
    ...
    if strcmp(Frequency_Band, 'beta')
     ...
     for rd = 1:length(session_info(:,1))
        ...
        savedir           = 'C:\Users\Corey Roach\Documents\00_DATA\Coh_Correct_Expectation_Visualization';  % Path to the parent directory where all new data will be stored (save structure: RecDate >> Animal >> Epoch >> all figures/files)
          ...
          % Initialize OnlyPretone_data.congruency as a cell array as a new selection factor for select data function  
            ...
            for b = 1:nIterations
                ...
                % Calculate PSI for both Conditions 
                    ...
                    % % save session value 
                    % save_file_name = sprintf('%s_%s_%s_%s_%s_PSI.mat', Animal, RecDate, 'OnlyPrior', 'correct' , Frequency_Band);
                    % save(fullfile(savedir, save_file_name), 'PSI_OnlyPrior');
                    % save_file_name = sprintf('%s_%s_%s_%s_%s_PSI.mat', Animal, RecDate, 'OnlyPretone', 'correct', Frequency_Band);
                    % save(fullfile(savedir, save_file_name), 'PSI_OnlyPretone')
             
                    save_file_name = sprintf('%s_%s_%s_%s_%s_Coh.mat', Animal, RecDate, 'OnlyPrior', 'correct',  Frequency_Band);
                    save(fullfile(savedir, save_file_name), 'Coh_OnlyPrior');
                    save_file_name = sprintf('%s_%s_%s_%s_%s_Coh.mat', Animal, RecDate, 'OnlyPretone', 'correct',  Frequency_Band);
                    ...
                    % clear_list = {'freq_OnlyPrior', 'freq_OnlyPretone', 'cfg','Coh_OnlyPrior', 'Coh_OnlyPretone'};
                    % clear(clear_list{:});
                    % clear clear_list;
