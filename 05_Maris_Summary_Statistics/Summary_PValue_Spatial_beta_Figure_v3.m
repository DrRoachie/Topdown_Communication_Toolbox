...
% %% Beta Statistics
% 
% % Compare Condition B (middle 8 rows) vs Condition A (top6 + bottom6)
% % Uses: ChanPair_Array_Coh_binom (20x20) with -log10(p) values
% 
% L10        = ChanPair_Array_Coh_binom;
% L10_thresh = -log10(0.05);          % exact cutoff ≈ 1.30102999566
% epsL       = 1e-12;                 % tiny tolerance
% 
% % Row sets
% rows_top6 = 1:6;
% rows_mid8 = 7:14;
...
% 
% % Build conditions
% A_vals = L10([rows_top6, rows_bot6], :);   % Condition A = top6 + bottom6
% B_vals = L10(rows_mid8, :);                % Condition B = middle8
% 
% % Flatten & threshold (include values exactly at the cutoff)
% A_keep = ~isnan(A_vals) & A_vals >= (L10_thresh - epsL);
% B_keep = ~isnan(B_vals) & B_vals >= (L10_thresh - epsL);
% 
% A_thr  = A_vals(A_keep);
% B_thr  = B_vals(B_keep);
% 
% % Sanity prints
% fprintf('Counts (suprathreshold): A(top+bottom)=%d, B(middle)=%d\n', numel(A_thr), numel(B_thr));
% if ~isempty(A_thr)
%     fprintf('  A range: min=%.6f  max=%.6f  median=%.3f  mean=%.3f\n', ...
%         min(A_thr), max(A_thr), median(A_thr), mean(A_thr));
% end
% if ~isempty(B_thr)
%     fprintf('  B range: min=%.6f  max=%.6f  median=%.3f  mean=%.3f\n', ...
%         min(B_thr), max(B_thr), median(B_thr), mean(B_thr));
% end
% 
% % If either group is empty, show near-threshold diagnostics
% if isempty(A_thr) || isempty(B_thr)
%     near = @(X) X(~isnan(X) & abs(X - L10_thresh) < 2e-3);
%     fprintf('Near-threshold counts (±0.002): A=%d, B=%d\n', numel(near(A_vals)), numel(near(B_vals)));
% end
% 
% % Proceed only if both groups have data
% if ~isempty(A_thr) && ~isempty(B_thr)
%     % Wilcoxon rank-sum: first input = B (middle), second = A (top+bottom)
%     % => stats.zval > 0 means B > A
%     [p_BA,~,stats_BA] = ranksum(B_thr, A_thr);
%     fprintf('\nWilcoxon (B vs A): z = %.2f, p = %.4f  (positive z => middle > top+bottom)\n', ...
%         stats_BA.zval, p_BA);
% 
...
%     bootDiff = nan(nBoot,1);
%     for b = 1:nBoot
%         bootDiff(b) = mean(datasample(B_thr, numel(B_thr))) - mean(datasample(A_thr, numel(A_thr)));
%     end
%     ci = prctile(bootDiff, [2.5 97.5]);
%     meanDiff = mean(B_thr) - mean(A_thr);
% 
%     fprintf('Mean diff (B - A) = %.3f, 95%% bootstrap CI [%.3f, %.3f]\n', meanDiff, ci(1), ci(2));
% else
%     warning('One of the groups is empty after thresholding. Consider checking coverage or threshold.');
% end
...
% %% Binomial test probability heatmap [DIAGONAL NAN]
% 
% ChanPair_Array_Coh_binom = nan(20,20);
% 
% for r = 1:20
...
% yc = linspace(yrange(1), yrange(2), s(1));
% 
% % Overlay dark navy for subthreshold; darker gray w/ diagonal for NaN
% darkNavy  = [0 0 0.35];
% darkGray  = [0.55 0.55 0.55];  % darker gray for NaN
% 
% for r = 1:s(1)
%     for c = 1:s(2)
%         if isnan(ChanPair_Array_Coh_binom(r,c))   % NaN case
%             % Darker gray box
%             rectangle('Position', [xc(c)-dx/2, yc(r)-dy/2, dx, dy], ...
%                       'FaceColor', darkGray, 'EdgeColor', 'k');
%             % Diagonal slash
%             plot([xc(c)-dx/2, xc(c)+dx/2], [yc(r)-dy/2, yc(r)+dy/2], ...
%                  'k-', 'LineWidth', 1.5);
%         elseif mask(r,c)   % Subthreshold (but not NaN)
%             rectangle('Position', [xc(c)-dx/2, yc(r)-dy/2, dx, dy], ...
%                       'FaceColor', darkNavy, 'EdgeColor', 'none');
%         end
%     end
