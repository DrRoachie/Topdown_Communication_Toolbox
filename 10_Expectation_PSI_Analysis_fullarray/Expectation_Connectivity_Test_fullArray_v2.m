...
% strong influence of SNR on PSI so we collapse across all levels of SNR
    ...
    %  end
     ...
     for rd = 1:length(session_info(:,1))
        ...
        chandir           = fullfile('\\Kilosort\d\Top_Down_Coherence_Project\00_DATA\05b_Signficant_Channels_epoch_fullArray', Animal, 'testToneOnset', RecDate);     % the sig channels that are output LFP_Spectral_Analysis
        savedir           = 'C:\Users\Corey Roach\Documents\00_DATA\PSI_Connectivity_FullArray_testTone_Cassius_WHOLE';  % Path to the parent directory where all new data will be stored (save structure: RecDate >> Animal >> Epoch >> all figures/files)
