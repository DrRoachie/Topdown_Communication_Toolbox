...
% Does the same thing as XCorr_Peak_Histogram but instead of taking all
% significant channels, it takes the max 3 channels from
...
datadir     = 'D:\03_Cohen_Lab\01_Top_Down_Coherence_Project\00_DATA\05_Epoc_Cut_2';
maxdir      = 'D:\03_Cohen_Lab\01_Top_Down_Coherence_Project\00_DATA\zz_MetaData\2024_07_01_Analysis\XCorr_Histogram_Data';
animals         = {'MrCassius', 'MrM'};       % 'MrCassius' and/or 'MrM'
Epoch           = 'testToneOnset';
...
for j = 1:length(animals)
    ...
    sessions = dir(fullfile(datadir, Animal, extractBefore(Epoch, 'Onset'), '19*'));

    for i = 1:length(sessions)
            
        RecDate = sessions(i).name;
        chandir = fullfile('D:\00_Significant_chans', Animal, extractBefore(Epoch, 'Onset'), RecDate);
        ...
        fName = sprintf('pvalue_max_3_channels_%s_%s_PFC_AC.mat', animal_lb, Frequency_Band);
        load(fullfile(maxdir, fName));
        max_results_PFC_AC = max_results;
        clear max_results;
    
        fName = sprintf('pvalue_max_3_channels_%s_%s_AC_PFC.mat', animal_lb, Frequency_Band);
        load(fullfile(maxdir, fName));
        ...
        for mr = 1:length(max_results_PFC_AC(:,1)) % transfer PFC to AC channel pairs from max results array to Significant_Channels array
            next_chan_pair = {sprintf('*PFC_ch%.2d', max_results_PFC_AC(mr,2)), sprintf('*AC_ch%.2d', max_results_PFC_AC(mr,3))}; % since the direction is PFC to AC, the sender column (2nd) is PFC and the receiver column (3rd) is AC
            Significant_Channels = [Significant_Channels; next_chan_pair];
        end
    
        for mr = 1:length(max_results_AC_PFC(:,1)) % transfer AC to PFC channel pairs from max results array to Significant_Channels array
            next_chan_pair = {sprintf('*PFC_ch%.2d', max_results_AC_PFC(mr,3)), sprintf('*AC_ch%.2d', max_results_AC_PFC(mr,2))}; % since the direction is AC to PFC, the sender column (2nd) is AC and the receiver column (3rd) is PFC
            Significant_Channels = [Significant_Channels; next_chan_pair]; % NOTE: XCorr calculation requires input to be PFC first and AC second, channel pair direction is flipped when adding to Significant_Channels
        end
    
        [C,ia] = unique(cell2mat(Significant_Channels),'rows'); % remove duplicate pairs (pairs that appear in max_results_PFC_AC and max_results_AC_PFC)
        Significant_Channels = Significant_Channels(ia,:);
        ...
        for k = 1:num_channels
            ...
            % calculate xcorr
            [shuffled_xcorr_result, shuffled_lagsResults, xcorr_result, lagsResults] = TrialbyTrialXcorr(datadir, Animal, RecDate, Epoch, Condition, Current_ChanPair);
...
file_name = 'xcorr_histogram_data_max_channels.mat';
