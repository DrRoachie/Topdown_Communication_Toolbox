...
% preCueOnset_PSI_grid, testToneOnset_PSI_grid, ...
%           preCue_averagedGrid, testTone_averagedGrid, ...
%           PFC_2_AC_PreCue, AC_2_PFC_PreCue, ...
%           PFC_2_AC_TestTone, AC_2_PFC_TestTone, ...
%           group_data
...
[~, ~, ~, ~, ...
 ~,~, ...
 OnlyPrior_PFC_2_AC_TestTone, OnlyPrior_AC_2_PFC_TestTone, ~] = build_epoch_PSI('OnlyPrior', animals, OnlyPrior_rootdir);

% --- Build (OnlyPretone) ---  (FIXED: condition + rootdir)
[~, ~, ~, ~, ...
 ~, ~, ...
OnlyPretone_PFC_2_AC_TestTone, OnlyPretone_AC_2_PFC_TestTone, ~] = build_epoch_PSI('OnlyPretone', animals, OnlyPretone_rootdir);

% --- Collapse across direction (PreCue): concatenate |PSI| from both directions ---
prior_pre_all   = [OnlyPrior_PFC_2_AC_TestTone(:);   OnlyPrior_AC_2_PFC_TestTone(:)];
pretone_pre_all = [OnlyPretone_PFC_2_AC_TestTone(:); OnlyPretone_AC_2_PFC_TestTone(:)];
...
fprintf('\nTestTone |PSI| (collapsed directions):\n');
...
baseName = 'TestTone_theta_Collapsed_AllPoints_4x4in';
