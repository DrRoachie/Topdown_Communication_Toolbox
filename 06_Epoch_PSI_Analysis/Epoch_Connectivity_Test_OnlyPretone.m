...
% 
    ...
    if strcmp(Frequency_Band, 'beta')
     ...
     for rd = 1:length(session_info(:,1))
        ...
        savedir           = 'C:\Users\Corey Roach\Documents\00_DATA\2025_19_12_PSI_Epoch_Analysis_OnlyPretone';  % Path to the parent directory where all new data will be stored (save structure: RecDate >> Animal >> Epoch >> all figures/files)
        ...
        % Load Correct Channels
        preCue_correct_SigChans_fn = sprintf('SigChannels_%s_%s_%s_%s_%s.mat', RecDate, 'preCueOnset', 'OnlyPretone', 'Correct', Frequency_Band);
        ...
        if ~isempty(preCue_correct_files)
            ...
            preCue_correct_PFC_chans = {};
            preCue_correct_AC_chans  = {};
        end
        
        % Load Wrong Channels
        preCue_wrong_SigChans_fn = sprintf('SigChannels_%s_%s_%s_%s_%s.mat', RecDate, 'preCueOnset', 'OnlyPretone', 'Wrong', Frequency_Band);
        ...
        if ~isempty(preCue_wrong_files)
            ...
            preCue_wrong_PFC_chans = {};
            preCue_wrong_AC_chans  = {};
        end
        
        % Combine channels from Correct and Wrong (if available)
        ...
        % Check if there are any channels to build pairs
        if isempty(combined_PFC_chans) && isempty(combined_AC_chans)
            % Both empty, generate null file and skip
            save_file_name = fullfile(savedir, sprintf('NULL_%s_%s_%s_%s_%s_data.txt', Animal, RecDate, 'OnlyPretone', Frequency_Band, 'NoChannels'));
            fid4 = fopen(save_file_name, 'w');
            fclose(fid4);
            warning('No PFC or AC channels found in both Correct and Wrong for %s %s', Animal, RecDate);
            continue; % skip to next session
        else
            % Build all pairwise combinations of available channels
            [PFC_comb, AC_comb] = ndgrid(combined_PFC_chans, combined_AC_chans);
            Shared_ChannelPairs = [PFC_comb(:), AC_comb(:)]; % Nx2 cell array
        end
        ...
        % 
        %     save_file_name = fullfile(savedir, sprintf('NULL_%s_%s_%s_%s_%s_%s_data.txt', Animal, RecDate, 'OnlyPretone', RecDate, Frequency_Band, 'Correct'));
        %     fid4 = fopen(save_file_name, 'w');
        %     fclose('all');
        % 
...
%% Calculate PSI Spectra during the testToneOnset epoch; PretoneOnly Trials, Collapsed Across Behavior (Correct and Wrong) and SNR (High and Low)
    ...
    for rd = 1:length(session_info(:,1))
        ...
        % Load Correct Channels
        testTone_correct_SigChans_fn = sprintf('SigChannels_%s_%s_%s_%s_%s.mat', RecDate, ' testToneOnset', 'OnlyPretone', 'Correct', Frequency_Band);
        ...
        if ~isempty( testTone_correct_files)
           ...
           testTone_correct_modifiedCellArray = regexprep(testTone_correct_SigChans.significant_channels, '^(D1_|D2_|D3_|D4_)', '*');
            ...
            testTone_correct_PFC_chans = {};
            testTone_correct_AC_chans  = {};
        end
        
        % Load Wrong Channels
       testTone_wrong_SigChans_fn = sprintf('SigChannels_%s_%s_%s_%s_%s.mat', RecDate, 'testToneOnset', 'OnlyPretone', 'Wrong', Frequency_Band);
       testTone_wrong_files = dir(fullfile(chandir, testTone_wrong_SigChans_fn));
        ...
        if ~isempty(testTone_wrong_files)
            ...
            testTone_wrong_PFC_chans = {};
            testTone_wrong_AC_chans  = {};
        end
