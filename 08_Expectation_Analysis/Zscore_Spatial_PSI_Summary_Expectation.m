...
rootdir           = 'C:\Users\Corey Roach\Documents\00_DATA\Expectation_Analysis\00_Core';                                 
    ...
    for j = 1:length(animals)
      ...
      for k = 1:length(PSI_OnlyPrior_file)
       ...
       OnlyPretone_labels   = PSI_OnlyPretone.labelcmb;  
        ...
        pooledData = [OnlyPrior_Matrix(:); OnlyPretone_Matrix(:)];
        ...
        Z_OnlyPrior_Matrix   = (OnlyPrior_Matrix   - globalMean) / globalStd;
        Z_OnlyPretone_Matrix = (OnlyPretone_Matrix - globalMean) / globalStd;
        zDiff_Matrix         = Z_OnlyPrior_Matrix - Z_OnlyPretone_Matrix;

      end    

           % Populate Z_OnlyPrior_PSI_grid with data 
           ...
           % Populate ZDiff_PSI_PSI_grid with data 
...
for pfcIdx = 1:20
    for acIdx = 1:20
        ...
        if ~isempty(Z_OnlyPrior_PSI_grid{pfcIdx, acIdx})
            ...
            dataposition = Z_OnlyPrior_PSI_grid{pfcIdx, acIdx};
            ...
            Z_OnlyPrior_averagedGrid(pfcIdx, acIdx) = columnAverage;
        end
    end
end
end
...
for pfcIdx = 1:20
    for acIdx = 1:20
        ...
        if ~isempty(Z_OnlyPretone_PSI_grid{pfcIdx, acIdx})
            % Extract the data matrix for the current position
            dataposition = Z_OnlyPretone_PSI_grid{pfcIdx, acIdx};

            % Compute the average of the selected columns
            columnAverage = mean(dataposition(:, columnRange), 'all', 'omitnan');

            % Store the average in the new grid
            Z_OnlyPretone_averagedGrid(pfcIdx, acIdx) = columnAverage;
        end
    end
end
end
...
for pfcIdx = 1:20
    for acIdx = 1:20
        ...
        if ~isempty(ZDiff_PSI_grid{pfcIdx, acIdx})
            % Extract the data matrix for the current position
            dataposition = ZDiff_PSI_grid{pfcIdx, acIdx};

            % Compute the average of the selected columns
            columnAverage = mean(dataposition(:, columnRange), 'all', 'omitnan');

            % Store the average in the new grid
            ZDiff_averagedGrid(pfcIdx, acIdx) = columnAverage;
        end
    end
end
end
