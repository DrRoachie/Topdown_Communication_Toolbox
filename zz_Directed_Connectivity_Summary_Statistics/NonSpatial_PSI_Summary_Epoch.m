...
rootdir    = 'C:\Users\Corey Roach\Documents\00_DATA\PSI_Epoch_Analysis\00_Belt';                                 
...
xlabel('AC Channels');
ylabel('PFC Channels');
title(['preCueOnset-',  Frequency_Band, '-', Animal]);
xticks(1:20);
yticks(1:20);
...
xlabel('AC Channels');
ylabel('PFC Channels');
title(['testToneOnset-',  Frequency_Band, '-', Animal]);
xticks(1:20);
yticks(1:20);
xticklabels(arrayfun(@(x) sprintf('ch%02d', x), 3:22, 'UniformOutput', false));
yticklabels(arrayfun(@(x) sprintf('ch%02d', x), 3:22, 'UniformOutput', false));
