...
% Channel labels identified through this spectral balancing procedure are saved 
% for use in subsequent PSI and coherence calculations. Connectivity metrics are 
% evaluated across spatial (e.g. PFC vs AC) and condition-based (e.g. OnlyPrior vs 
% OnlyPretone) groupings.
...
%% Session wise generate the meta data 
     ...
     for rd = 1:length(session_info(:,1))
        ...
        savedir           = 'C:\Users\auditory research la\OneDrive\Desktop\SpecBalanced_Eval_Overlap_Condition';                                               % Path to the parent directory where all new data will be stored (save structure: RecDate >> Animal >> Epoch >> all figures/files)
        ...
        % extract session information
         ...
         % indicates congruence between target and LED
          ...
          % Initialize OnlyPretone_data.congruency as a cell array as a new selection factor for select data function  
            ...
            figure;
            hold on;
            histogram(theta_power_OnlyPrior, bin_edges, 'FaceAlpha', 0.5, 'DisplayName', 'OnlyPrior-all vals');
            histogram(theta_power_OnlyPretone, bin_edges, 'FaceAlpha', 0.5, 'DisplayName', 'OnlyPretone-all vals');
            ...
            % Save histogram
            saveas(gcf, fullfile(savedir, sprintf('Overlap_OnlyPrior_OnlyPretone_%s_%s.png', RecDate, Animal)));
            close(gcf);

            labels_a = tfreq_OnlyPrior.label;
            labels_b = tfreq_OnlyPretone.label;
            ...
            % === Save the struct ===
            save(fullfile(savedir, sprintf('OverlappingLabelsConditions_%s_%s.mat', RecDate, Animal)), 'OverlappingLabelsConditions');

            
            % labels_a = tfreq_OnlyPrior.label  ;
            % labels_b = tfreq_OnlyPretone.label ;
            % 
            % [OnlyPrior_labels_a_out, OnlyPretone_labels_b_out] = SpecBalancedRangeBased(theta_power_OnlyPrior, theta_power_OnlyPretone, labels_a, labels_b);
            % 
            % % Get labels from SpecBalancedRangeBased
            % labels_a = tfreq_OnlyPrior.label;
            % labels_b = tfreq_OnlyPretone.label;
            % [OnlyPrior_labels_out, OnlyPretone_labels_out] = SpecBalancedRangeBased(theta_power_OnlyPrior, theta_power_OnlyPretone, labels_a, labels_b);
            % 
            % % Save output as struct
            % OverlappingLabelsConditions = struct();
            % OverlappingLabelsConditions.OnlyPrior = OnlyPrior_labels_out(:);
            % OverlappingLabelsConditions.OnlyPretone = OnlyPretone_labels_out(:);
            % 
            % save(fullfile(savedir, sprintf('OverlappingLabelsConditions_%s_%s.mat', RecDate, Animal)), 'OverlappingLabelsConditions');


            %% === Calling SpecBalancedRangeBased === (Overlap between Areas)

                % Channel labels (same across sessions)
                                            
                % pfc_labels = arrayfun(@(x) sprintf('*PFC_ch%02d', x), 3:22, 'UniformOutput', false);  % <<< FIXED (has zero padding)
                % ac_labels  = arrayfun(@(x) sprintf('*AC_ch%02d', x),  3:22, 'UniformOutput', false);  % <<< FIXED (has zero padding)
                % pfc_idx = 1:20;
                % ac_idx = 21:40; 
                % 
